...
with socketserver.TCPServer(('127.0.0.1', 0), H) as srv:
    ...
    with sync_playwright() as p:
        ...
        # D7: krok zpět, hromadná změna, přečíslování, karta bodu, koš
        pg.click('[data-tab=body]'); n0 = pg.locator('#body-tab tbody tr').count()
        pg.click('text=+ Bod'); pg.fill('#nb-c', '777'); pg.fill('#nb-y', '745010'); pg.fill('#nb-x', '1045010'); pg.click('.dlg-pata >> text=Přidat'); pg.wait_for_timeout(200)
        ok(pg.locator('#body-tab tbody tr').count() == n0 + 1, 'bod 777 přidán')
        pg.click('#btn-zpet'); pg.wait_for_timeout(200); ok(pg.locator('#body-tab tbody tr').count() == n0, 'krok zpět odstranil bod 777')
        pg.click('#btn-vpred'); pg.wait_for_timeout(200); ok(pg.locator('#body-tab tbody tr').count() == n0 + 1, 'krok vpřed ho vrátil')
        pg.click('#sekce-body >> text=⋯'); pg.click('text=Hromadná změna'); pg.check('#hz-z-y'); pg.fill('#hz-v-y', '10'); pg.select_option('#hz-o-y', 'pricist'); pg.click('.dlg-pata >> text=Provést'); pg.wait_for_timeout(200)
        y777 = pg.locator('#body-tab tbody tr:has(input[value="777"]) input[aria-label=y]').input_value(); ok(y777 == '745020,000', 'hromadná změna Y+10: ' + y777)
        pg.click('#sekce-body >> text=⋯'); pg.click('text=Přečíslování'); pg.select_option('#pc-rezim', 'doplnit'); pg.fill('#pc-pre', '61084400014'); pg.uncheck('#pc-ulozit'); pg.click('.dlg-pata >> text=Přečíslovat'); pg.wait_for_timeout(300)
        ok(pg.locator('#body-tab tbody tr:has(input[value="610844000140777"])').count() == 1, 'přečíslování doplnilo předčíslí: 777 → 610844000140777')
        pg.click('#btn-zpet'); pg.wait_for_timeout(200); ok(pg.locator('#body-tab tbody tr:has(input[value="777"])').count() == 1, 'zpět vrátilo čísla')
        pg.locator('#body-tab tbody tr:has(input[value="777"]) button.ikona').click(); pg.wait_for_selector('#kb-cislo'); pg.check('#kb-zamek'); pg.select_option('#kb-kvalita', '3'); pg.click('.dlg-pata >> text=Uložit'); pg.wait_for_timeout(200)
        ok('🔒' in pg.locator('#body-tab tbody tr:has(input[value="777"])').inner_text(), 'karta bodu: zámek')
        pg.locator('#body-tab tbody tr:has(input[value="777"]) button.ikona').click(); pg.wait_for_selector('#kb-cislo'); pg.click('.dlg-pata >> text=Smazat'); pg.wait_for_timeout(300)
        ok(pg.locator('#body-tab tbody tr:has(input[value="777"])').count() == 1, 'zamčený bod nejde smazat')
        pg.keyboard.press('Escape'); pg.locator('#body-tab tbody tr:has(input[value="777"]) button.ikona').click(); pg.wait_for_selector('#kb-cislo'); pg.uncheck('#kb-zamek'); pg.click('.dlg-pata >> text=Uložit'); pg.wait_for_timeout(200)
        pg.locator('#body-tab tbody tr:has(input[value="777"]) button.ikona').click(); pg.wait_for_selector('#kb-cislo'); pg.click('.dlg-pata >> text=Smazat'); pg.click('.dlg-pata >> text=Ano'); pg.wait_for_timeout(300)
        ok(pg.locator('#body-tab tbody tr:has(input[value="777"])').count() == 0, 'odemčený bod smazán do koše')
        pg.click('#sekce-body >> text=⋯'); pg.click('text=Koš'); pg.check('.dlg-telo input[type=checkbox]'); pg.click('.dlg-pata >> text=Obnovit'); pg.wait_for_timeout(300)
        ok(pg.locator('#body-tab tbody tr:has(input[value="777"])').count() == 1, 'obnoven z koše')
